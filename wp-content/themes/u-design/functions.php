...
if ( ! function_exists( 'who_can_edit_udesign_theme_options' ) ) {
	 ...
	 * More on roles and capabilities: http://codex.wordpress.org/Roles_and_Capabilities
	 * 
...
if ( function_exists( 'load_theme_textdomain' ) ) {
	load_theme_textdomain( 'udesign', get_template_directory() . '/lang' );
}
...
function udesign_init_styles() {
	...
	wp_enqueue_style('u-design-style', get_template_directory_uri() . "/assets/css/frontend/global/style.css", false, UDESIGN_VERSION, 'screen');
        ...
        // This option makes sure the CSS file "style.css" in the theme's root folder is not needlessly being loaded
 ...
 */
include ( trailingslashit( get_template_directory() ) . 'inc/admin/shortcodes/u-design-shortcodes.php' );
...
/* Custom widgets... */
include ( trailingslashit( get_template_directory() ) . 'inc/shared/widgets/loginForm-widget.php' );
include ( trailingslashit( get_template_directory() ) . 'inc/shared/widgets/customCategory-widget.php' );
include ( trailingslashit( get_template_directory() ) . 'inc/shared/widgets/googleMap-widget.php' );
include ( trailingslashit( get_template_directory() ) . 'inc/shared/widgets/latestPost-widget.php' );
include ( trailingslashit( get_template_directory() ) . 'inc/shared/widgets/subpages-widget.php' );
...
if ( $udesign_options['enable_feedback'] ) {
    function udesign_feedback_button() {
		...
		if ( isset( $udesign_options['feedback_btn_text'] ) && $udesign_options['feedback_btn_text'] ) :
			?>
			<div id="feedback"><a href="<?php echo $udesign_options['feedback_url']; ?>" class="feedback"><?php echo $udesign_options['feedback_btn_text']; ?></a></div>
			<?php
		endif;
...
if ( $udesign_options['enable_page_peel'] ) {
    function udesign_enable_page_peel() {
        ...
        ob_start(); ?>
            <div id="page-peel">
                    <a href="<?php echo $udesign_options['page_peel_url']; ?>" title="<?php esc_attr_e('Subscribe', 'udesign'); ?>"><img src="<?php echo esc_url( get_template_directory_uri() ); ?>/assets/images/page_peel.png" alt="<?php esc_attr_e('Subscribe', 'udesign'); ?>" /></a>
...
function  udesign_get_comments_link() {
	...
	if ( ! post_password_required() && ( comments_open() ) ) {
		ob_start();
		?>
			<span class="postmetadata-comments-link"> / &nbsp; <?php comments_popup_link( __( 'Leave a comment', 'udesign' ), __( '1 Comment', 'udesign' ), __( '% Comments', 'udesign' ) ); ?></span>
		<?php
...
function udesign_footer_back_to_top_link() {
	...
	?>
		<?php if ( 'angle-up' === $udesign_options['back_to_top'] ) : ?>
			<a href="#" id="back-to-top-fixed" style="display: none;"><span class="<?php echo $angle_up_icon_source; ?>"></span><span class="back-to-top">Back to Top</span></a>
		...
		<style type="text/css">
			@media screen and (min-width: 960px) {
				...
				<?php else : ?>
					...
					@media screen and (min-width: <?php echo ( $udesign_custom_page_width + 140 ); ?>px) {
						.udesign-is-custom-page-width #main-content .alignwide {
							margin-left: -70px;
							margin-right: -70px;
							max-width: <?php echo ( $udesign_custom_page_width + 100 ); ?>px;
						}
						...
						.udesign-is-custom-page-width #main-content .wp-block-pullquote.alignwide {
							width: <?php echo ( $udesign_custom_page_width + 100 ); ?>px;
						}
					}
				<?php endif; ?>
				/* Sidebar. */
...
function udesign_add_admin_bar() {
	...
	$args = array(
		...
		'parent' => 'u-design-settings-page',
		'title'  => __( 'Backup / Restore', 'udesign' ),
	...
	$wp_admin_bar->add_node( $args );
        ...
        if ( class_exists( 'Merlin' ) ) {
            $args = array(
                    ...
                    'parent' => 'u-design-settings-page',
                    'title'  => __( 'Import Demo Data', 'udesign' ),
...
if ( ! function_exists( 'udesign_gutenberg_setup' ) ) {
	...
	function udesign_gutenberg_setup() {
		...
		add_theme_support( 'editor-font-sizes', array (
				array(
					'name' => __( 'small', 'udesign' ),
					'shortName' => __( 'S', 'udesign' ),
					'size' => $udesign_options['general_font_size'] - 2,
					'slug' => 'small'
				),
				array(
					'name' => __( 'normal', 'udesign' ),
					'shortName' => __( 'N', 'udesign' ),
					'size' => $udesign_options['general_font_size'],
					'slug' => 'normal'
				),
				array(
					'name' => __( 'medium', 'udesign' ),
					'shortName' => __( 'M', 'udesign' ),
					'size' => $udesign_options['general_font_size'] + 4,
					'slug' => 'medium'
				),
				array(
					'name' => __( 'large', 'udesign' ),
					'shortName' => __( 'L', 'udesign' ),
					'size' => $udesign_options['general_font_size'] + 20,
					'slug' => 'large'
				),
				array(
					'name' => __( 'huge', 'udesign' ),
					'shortName' => __( 'XL', 'udesign' ),
...
function udesign_gutenberg_editor_styles() {
	...
	// Make some of theme's custom styles available to Gutenberg editor.
	require_once get_parent_theme_file_path( '/assets/css/admin/gutenberg-dynamic-editor-styles.php' );
	wp_add_inline_style( 'u-design-block-editor-styles', udesign_dynamic_block_editor_styles() );
