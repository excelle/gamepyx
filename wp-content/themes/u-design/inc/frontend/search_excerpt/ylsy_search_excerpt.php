 ...
 * Description: Modify the_exceprt() template code during search to return snippets containing the search phrase. 
 * Snippet extraction code from Drupal's (http://drupal.org/) search module. And patched by Jam to support Asian text.
 * 
 * Version: 1.2 $Rev$
 * Author: Scott Yang
...
class SearchExcerpt {
	function get_content() {
		/**
		 * Get the content of current post. We like to have the entire content. 
		 * If we call get_the_content() we'll only get the teaser + page 1.
		 */
		global $post;

		/**
		 * Password checking copied from template-functions-post.php/get_the_content()
...
function _jamul_find_delimiter( $string, $pos = 0, $min = -1, $max = -1 ) {
	...
	if ( ord( $string[ $pos ] ) < 0x80 ) {
		/**
		 * Found ASCII character at the trimming point.  So, trying to find new trimming point around $pos. 
		 * New trimming point should be on a whitespace or the transition from ASCII to other character.
		 */
		$pos3 = -1;
...
/**
 * Add with priority=5 to ensure that it gets executed before wp_trim_excerpt in default filters.
 */
